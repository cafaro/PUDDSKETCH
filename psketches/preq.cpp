...
using sketch_type = datasketches::req_sketch<double>;
...
void print_usage(char *prog_name) {
  printf("\nUsage: %s -f <input file name>\n"
         "    Options\n"
         "      -k <double>: parameter K for sketch. Default 12.0\n"
...
int main(int argc, char *argv[])
{
    ...
    opts opts{12.0, 1000, "unif", 1.0, 1000.0, 0, false};
    int opt;
    ...
    int B = 2*opts.k*std::ceil(std::log2(opts.loc_n/opts.k));
    int N = std::ceil(std::log2(opts.loc_n/B))+1;
    theo_max_size = N*B*sizeof(double);
    ...
    if (!pid) {
        ...
        res.sketch_size = *(size_t*)merged_sketch_bytes;
...
void print_stats(const opts& o, const results& r) {
    if (!o.csv) {
        std::cout << "Parallel REQ algorithm:\n"
                  << "K: " << o.k << "\n"
