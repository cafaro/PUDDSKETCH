...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    double additionalTime = 0.0;        // time (seconds) spent for the additional collapses procedure before the Reduce -- UDD-only 
    ...
    double maxAdditionalTime = 0.0;         // max of times (seconds) spent for performing additional Collapses -- UDD-only
    ...
    for(long i = 0; i < local_bsize; ++i) {
        ...
        #ifdef UDD
            while ((posibins + negabins) > configParam.sketch_bound) {
            
                collapseUniformly(posiSketch);
                posibins = posiSketch.size();
                    
                collapseUniformly(negaSketch);
                negabins = negaSketch.size();
                    
                //update process's alpha
                p_alpha = 2*(p_alpha/(1 + pow(p_alpha,2)));
                        
                // gamma and base must be updated to the current alpha
                gamma = (1+p_alpha)/(1-p_alpha);
                base = std::log10(gamma);

                //update the number of collapse executed
                ++process_collapses;
    ...
    int collapsesA = 0;       //loop counter
        ...
        additionalTime -= MPI_Wtime();
            ...
            //*** Step 10.1: each process exchanges its current value of (DDSketch's α)/(collapses executed)
            //double max_alpha = 0;                 // the max α choosed among all the MPI processes
            int max_collapses = 0;                  // the max # of collapses executed, choosed among all the MPI processes
            MPI_Allreduce(&process_collapses, &max_collapses, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD); //MPI_Allreduce(&p_alpha, &max_alpha, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);

            #ifdef LOG
                if (f_log) {
                    fprintf(f_log, "\tP%d) local α=%.6f, local collapses=%d, max %d\n", process_rank, p_alpha, process_collapses, max_collapses);
                }
            #endif

            //*** Step 10.2: additional collapses until the (max α)/(max collapses)
            // @note: check against collapses instead of α, to avoid doubles approximation errors
            
            
            while (process_collapses < max_collapses) { // while (p_alpha < max_alpha) { 
                ...
                monitorCollapses[m_idx].collapse = process_collapses;
                monitorCollapses[m_idx].items = negapop + posipop;
                //++m_idx;
            
            }//wend additional collapses

        additionalTime += MPI_Wtime();
        
        //get the max of the times for executing the additional collapses
        MPI_Allreduce(&additionalTime, &maxAdditionalTime, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD); 


        #ifdef LOG
            if (f_log) {
                fprintf(f_log, "P%d executed additional %d collapses before MPI_Reduce (total collapses %d)\nValue of alpha: %.6f\n", process_rank, collapsesA, process_collapses, p_alpha);
            }
        #endif
        #ifndef NDEBUG
            // before MPI_Reduce, the total number of local collapses (in process_collapses) is max_collapses
            assert(process_collapses==max_collapses);
        #endif
    ...
    mergeTime -= MPI_Wtime();
        ...
        #ifdef UDD
            char *localSketch = pack_SketchesPair(p_alpha, process_collapses, configParam.sketch_bound, posiSketch, posibins, posipop, negaSketch, negabins, negapop, &Gbuffer_size); 
        #else
        ...
        #ifdef UDD
            MPI_Op_create(reduceSketchPair, commute, &custom_merge);    //*** UDD
        #else
    ...
    if (!process_rank) 
    {
        ...
        fprintf(stderr,"%ld,%d,%d,%.6f,", totalInputSize, comm_size, configParam.sketch_bound, configParam.initial_alpha);
            ...
            // final Alpha, Tot Collapses, q0-bound, q1-bound
            fprintf(stderr,"%.6f,%d,%.3f,%.3f,", globalSketch->alpha, globalSketch->collapses, 0.0, 1.0);
